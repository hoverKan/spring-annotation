 ...
 *
 *  AbstractAutoProxyCreator.setBeanFactory() --》 initBeanFactory()
 ...
 *                      4、applyBeanProcessorsAfterInitialization:执行后置处理器的postProcessAfterInitialization()
 ...
 *          2、创建bean
 *             [AnnotationAwareAspectJAutoProxyCreator 在所有的bean创建之前会有一个拦截,
 *              InstantiationAwareBeanPostProcessor,会调用 postProcessBeforeInstantiation]
 *             1、先从缓存中获取当前bean，如果能获取到，说明bean之前被创建过，可以直接使用
 *                  只要创建好的Bean都会被缓存起来
 *             2、createBean();创建bean；
 ...
 *                          if(bean != null){
 *                              bean = applyBeanProcessorsAfterInitialization(beam,beanName);
 *                          }
 *                      2、doCreateBean(beanName,mbdRoUse,args);真正的去创建一个bean实例；和3.6流程一样
 *
 *
 ...
 *     3、是否需要跳过
 *          1、获取候选的增强器(切面里面的增强方法)【List<Advisor> candidateAdvisors】
 ...
 *          1、找到候选的所有的增强器（找哪些方法是需要切入当前bean 方法的）
 *          2、获取到能在bean使用的增强器
 *          3、给增强器排序
 *      2、保存当前bean在advisedBeans中
 *      3、如果当前的bean需要增强，创建当前的bean的代理对象
 *          1、获取所有的增强器（通知方法）
 *          2、保存到proxyFactory
 *          3、创建代理对象：Spring自动决定
 ...
 *
 * 3.目标方法执行
 *      容器中保存了组建的代理对象（cglib增强后的对象），这个对象里面保存了详细信息(比如增强器，目标对象，xxx)
 ...
 *      4、如果有拦截器链，把需要执行的目标对象，目标方法，拦截器链等信息传入一个CglibMethodInvocation 对象
